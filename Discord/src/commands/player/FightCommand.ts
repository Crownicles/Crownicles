...
} from "../../utils/KeycloakPlayerUtils";
...
export async function handleEndOfFight(context: PacketContext, packet: CommandFightEndOfFightPacket): Promise<void> {
	...
	let description = i18n.t("commands:fight.end.gameStats", {
		...
		time: i18n.formatDuration(millisecondsToMinutes(asMilliseconds(new Date().valueOf() - interaction.createdTimestamp)), lng)
	});
...
export async function handleFightReward(context: PacketContext, packet: FightRewardPacket): Promise<void> {
	...
	const player1Username = await resolveKeycloakPlayerName(packet.player1.keycloakId, lng);
	const player2Username = await resolveKeycloakPlayerName(packet.player2.keycloakId, lng);
