...
import { ChestPlantHandler } from "./ChestPlantHandler";
...
export class ChestFeatureHandler implements HomeFeatureHandler {
	...
	private readonly plantHandler = new ChestPlantHandler({
		buildItemButton: this.buildItemButton.bind(this),
		buildV2Container: this.buildV2Container.bind(this),
		createChestCollector: this.createChestCollector.bind(this),
		refreshChestCategoriesMenu: this.refreshChestCategoriesMenu.bind(this)
	});
	...
	private async handlePrefixAction(
		...
		// Plant-related actions (deposit, withdraw, pagination, tab navigation)
		if (await this.plantHandler.handlePlantInteraction(ctx, selectedValue, componentInteraction, nestedMenus)) {
			return true;
		}
