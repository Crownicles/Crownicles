...
export function PetSellCollector({collector, onChoose, submitting}: {collector: ReactionCollectorCreation; onChoose: (index: number) => void; submitting: boolean}): ReactNode {
	...
	const close = (): void => {if (offer.role !== PET_SALE_ROLES.OBSERVER) answer(refuseIndex);};
	return <Confirmation title={i18n.t("app:pet.sale.offerTitle")} onRequestClose={close}>
		<Note>{`${petIcon(offer.pet)} ${petName(offer.pet)}`}</Note>
		<SaleParties offer={offer} />
		{offer.role === PET_SALE_ROLES.SELLER ? <Note>{i18n.t("app:pet.sale.waiting")}</Note> : null}
		<ButtonRow>
			{offer.role === PET_SALE_ROLES.BUYER ? <Button variant="primary" disabled={locked || acceptIndex < 0} onPress={(): void => answer(acceptIndex)}>{i18n.t("app:pet.sale.buy")}</Button> : null}
			{offer.role !== PET_SALE_ROLES.OBSERVER ? <Button disabled={locked || refuseIndex < 0} onPress={close}>{i18n.t(offer.role === PET_SALE_ROLES.SELLER ? "app:pet.sale.cancel" : "app:pet.sale.refuse")}</Button> : null}
		</ButtonRow>
