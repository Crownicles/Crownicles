...
export class WebSocketClient {
	...
	private connectionSuccessCallback: (() => void) | null = null;
	...
	public setConnectionSuccessCallback(callback: () => void): void {
		this.connectionSuccessCallback = callback;
	}
	...
	public sendPacket(packet: FromClientPacket, responseHandlers: {
		...
		if (Object.keys(responseHandlers).length > 0) {
			const packetId = crypto.randomUUID(); // Generate a unique ID for the packet
	...
	private async connect(): Promise<void> {
		...
		this.socket = new WebSocket(`${process.env.EXPO_PUBLIC_WEB_SOCKET_URL}?token=${await authToken.getAccessTokenAndRefreshIfNeeded()}`);

		this.socket.onopen = (): void => {
			console.log("WebSocket connection established.");
			this.connectionAttempts = 0; // Reset connection attempts on successful connection
			this.connecting = false;
			if (this.connectionSuccessCallback) {
