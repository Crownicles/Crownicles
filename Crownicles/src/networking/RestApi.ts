...
export class RestApi {
	private static getBaseUrl(): string {
		if (!process.env.EXPO_PUBLIC_REST_API_URL) {
			throw new Error("REST_API_URL is not defined in the environment variables.");
		}
		return process.env.EXPO_PUBLIC_REST_API_URL;
	}
	...
	public static loginWithDiscord(): Promise<WebBrowser.WebBrowserAuthSessionResult> {
		return WebBrowser.openAuthSessionAsync(`${RestApi.getBaseUrl()}/discord`);
	}
