...
describe("Pet feed cooldown validation", () => {
	...
	it("should validate that BREED_COOLDOWN constant is set to 30 minutes", () => {
		...
		// Check that BREED_COOLDOWN is set to minutesToMilliseconds(asMinutes(30))
		const cooldownMatch = constantsContent.match(/BREED_COOLDOWN\s*=\s*minutesToMilliseconds\(\s*asMinutes\(\s*(\d+)\s*\)\s*\)/);
		expect(cooldownMatch).not.toBeNull();
		
		if (cooldownMatch) {
			const minutes = parseInt(cooldownMatch[1]);
			expect(minutes * 60 * 1000).toBe(EXPECTED_BASE_COOLDOWN);
		}
	});
});
