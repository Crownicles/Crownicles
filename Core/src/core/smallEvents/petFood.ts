...
import { makePacket, CrowniclesPacket, PacketContext } from "../../../../Lib/src/packets/CrowniclesPacket";
...
function getFoodType(player: Player): string {
	...
	if (SmallEventConstants.PET_FOOD.BAD_SMELL_TYPES.includes(type)) probabilities = SmallEventConstants.PET_FOOD.PROBABILITIES.BAD_SMELL;
	else if (SmallEventConstants.PET_FOOD.VEGETARIAN_TYPES.includes(type)) probabilities = SmallEventConstants.PET_FOOD.PROBABILITIES.VEGETARIAN;
	else if (SmallEventConstants.PET_FOOD.MEAT_TYPES.includes(type)) probabilities = SmallEventConstants.PET_FOOD.PROBABILITIES.MEAT;
	else if (SmallEventConstants.PET_FOOD.GOOD_SMELL_TYPES.includes(type)) probabilities = SmallEventConstants.PET_FOOD.PROBABILITIES.GOOD_SMELL;
	...
	for (const [foodType, probability] of Object.entries(probabilities)) {
		...
		if (rand < cumulativeProbability) return foodType;
	}
...
async function applyOutcome(player: Player, foodType: string, outcome: string, response: CrowniclesPacket[], properties: PetFoodProperties): Promise<void> {
	...
	if (outcome === "found_by_player" || outcome === "found_by_pet" || outcome === "found_anyway") {
		switch (foodType) {
			case SmallEventConstants.PET_FOOD.FOOD_TYPES.BAD_SMELL:
				const rand = RandomUtils.crowniclesRandom.realZeroToOneInclusive();
				if (rand < properties.probabilities.badSmell.plus) loveChange = properties.love.badSmell.plus;
				else if (rand < properties.probabilities.badSmell.plus + properties.probabilities.badSmell.minus) loveChange = properties.love.badSmell.minus;
				else loveChange = 0;
				break;
			...
			case SmallEventConstants.PET_FOOD.FOOD_TYPES.VEGETARIAN:
				if (petModel.diet === PetDiet.CARNIVOROUS) loveChange = 0;
				else loveChange = properties.love.vegetarian;
				break;
			case SmallEventConstants.PET_FOOD.FOOD_TYPES.MEAT:
				if (petModel.diet === PetDiet.HERBIVOROUS) loveChange = 0;
				else loveChange = properties.love.meat;
				break;
	...
	if (loveChange !== 0) {
		...
		if (petEntity.lovePoints < 0) petEntity.lovePoints = 0;
	...
	return async (collector, response) => {
		...
		if (reaction) {
			if (reaction.reaction.type === ReactionCollectorPetFoodInvestigateReaction.name) {
				...
				let delay = Math.max(0, diffHours);
