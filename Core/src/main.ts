...
function isBlockedByMaintenance(dataJson: IncomingPacketData): boolean {
	return botConfig.MODE_MAINTENANCE
		&& !CoreConstants.BYPASS_MAINTENANCE_AND_RESETS_PACKETS.includes(dataJson.packet.name)
		&& !dataJson.context.rightGroups?.includes(RightGroup.MAINTENANCE)
		&& !dataJson.context.rightGroups?.includes(RightGroup.ADMIN);
}

function isBlockedByReset(dataJson: IncomingPacketData): boolean {
	return resetIsNow()
		&& !CoreConstants.BYPASS_MAINTENANCE_AND_RESETS_PACKETS.includes(dataJson.packet.name);
}

function globalStopOfPlayers(response: CrowniclesPacket[], dataJson: IncomingPacketData): boolean {
	if (isBlockedByMaintenance(dataJson)) {
		response.push(makePacket(ErrorMaintenancePacket, {}));
		return true;
	}
	if (isBlockedByReset(dataJson)) {
